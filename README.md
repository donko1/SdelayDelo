...
7. Create local settings and ($${\color{red}Important}$$) configure it:
...
7. Create local settings(rename SdelayDelo\local_setting.py.example to SdelayDelo\local_setting.py ) and ($${\color{red}Important}$$) configure it:
...
3. Explore the API: Use tools like Postman or curl to interact with the API endpoints. Pay attention to the required data formats for each endpoint. The icon API expects data as form data. U can see examples of usage in [this](https://github.com/donko1/SdelayDelo/blob/main/tasks/tests.py) file
